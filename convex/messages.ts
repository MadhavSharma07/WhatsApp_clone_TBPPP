...
export const sendDocs = mutation({
	args: { DocsId: v.id("_storage"), sender: v.id("users"), conversation: v.id("conversations") },
	handler: async (ctx, args) => {
		const identity = await ctx.auth.getUserIdentity();
		if (!identity) {
			throw new ConvexError("Unauthorized");
		...
		const content = (await ctx.storage.getUrl(args.DocsId)) as string;
